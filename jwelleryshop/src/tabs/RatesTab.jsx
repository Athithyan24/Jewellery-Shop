          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200 text-sm text-left">
              <thead className="bg-slate-800 sticky top-0 z-10 shadow-md">
                <tr>
                  <th className="py-3 px-6 text-xs font-bold text-white uppercase tracking-wider">புகைப்படம்</th>
                  <th className="py-3 px-6 text-xs font-bold text-white uppercase tracking-wider">பெயர் (Name)</th>
                  <th className="py-3 px-6 text-xs font-bold text-white uppercase tracking-wider">Loan ID</th>
                  <th className="py-3 px-6 text-xs font-bold text-white uppercase tracking-wider">தேதி (Date)</th>
                  <th className="py-3 px-6 text-xs font-bold text-white uppercase tracking-wider">அடகு பொருள் (Product)</th>
                  <th className="py-3 px-6 text-xs font-bold text-white uppercase tracking-wider text-center">முதல் தவணை</th>
                  <th className="py-3 px-6 text-xs font-bold text-white uppercase tracking-wider text-center">இரண்டாம் தவணை</th>
                  <th className="py-3 px-6 text-xs font-bold text-white uppercase tracking-wider text-center">மூன்றாம் தவணை</th>
                  <th className="py-3 px-6 text-xs font-bold text-white uppercase tracking-wider text-right">தற்போதைய மாத நிலை</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-slate-100">
                {filteredLoans && filteredLoans.length > 0 ? (
                  filteredLoans.map((loan) => {
                    // Calculate days simply to know if a tier is active/upcoming visually
                    const createdDate = new Date(loan.createdAt);
                    const today = new Date();
                    const diffDays = Math.ceil(Math.abs(today - createdDate) / (1000 * 60 * 60 * 24));
                      ...
                      >
                        ...
                        <td className="py-3 px-4 whitespace-nowrap">
                          <img
                            ...
                            onError={(e) => {
                              e.target.src = "https://ui-avatars.com/api/?name=" + (loan.customer?.name || "User") + "&background=F1F5F9&color=64748B";
                            }}
                          />
                        </td>
  ...
  {!loan.isClosed ? (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <div className="text-[10px] font-bold text-slate-500 bg-white border border-slate-200 px-2 py-1 rounded-md shadow-sm">
        {formatTierDate(loan.createdAt, 0)} - {formatTierDate(loan.createdAt, t1End)}
      </div>
    </div>
  ) : (
  ...
  {!loan.isClosed ? (
    <div className="flex flex-col items-center gap-1.5">
      {diffDays > t1End ? (
        ...
        <span className="font-mono font-bold text-slate-300 px-2 py-0.5 text-sm">-</span>
      )}
      <div className="text-[10px] font-bold text-slate-500 bg-white border border-slate-200 px-2 py-1 rounded-md shadow-sm">
        {formatTierDate(loan.createdAt, t1End + 1)} - {formatTierDate(loan.createdAt, t2End)}
      </div>
    </div>
  ) : (
    <span className="text-slate-300 font-bold">-</span>
...
<td className="py-3 px-4 whitespace-nowrap text-center">
  {!loan.isClosed ? (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <div className="text-[10px] font-bold text-slate-500 bg-white border border-slate-200 px-2 py-1 rounded-md shadow-sm">
        {formatTierDate(loan.createdAt, t2End + 1)} - {formatTierDate(loan.createdAt, t3End)}
      </div>
    </div>
  ) : (
    <span className="text-slate-300 font-bold">-</span>
                        ...
                        {/* 9. Total Pending / Paid Interest (Exact logic from your backend/LoansTab) */}
                  ...
                  <tr>
                    <td colSpan="9" className="py-12 px-6 text-center text-slate-500 font-medium">
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <p>எந்த வட்டி விவரங்களும் இல்லை (No interest records found matching your search).</p>
                      </div>
                    </td>
                  </tr>
                )}
